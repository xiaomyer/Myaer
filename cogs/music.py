...
from core.music.video import Video
...
async def audio_playing(ctx):
    ...
    if client and client.channel and client.source:
        ...
        await ctx.send(embed=discord.Embed(
            ...
            description="Not currently playing any audio."
        ))
...
async def in_voice_channel(ctx: commands.Context):
    ...
    if voice and bot_voice and voice.channel and bot_voice.channel and voice.channel == bot_voice.channel:
        ...
        await ctx.send(embed=discord.Embed(
            color=ctx.author.color,
            timestamp=ctx.message.created_at,
            description="You must be connected to a voice channel to do that"
        ))
...
async def is_audio_requester(ctx):
    ...
    if permissions.administrator or state.is_requester(ctx.author):
        ...
        await ctx.send(embed=discord.Embed(
            color=ctx.author.color,
            timestamp=ctx.message.created_at,
            description="You must be the song requester to do that"
        ))
...
class Music(commands.Cog):
    ...
    async def unpause(self, ctx):
        """Unpauses any currently playing audio."""
        client = ctx.guild.voice_client
        self._unpause_audio(client)

    def _unpause_audio(self, client):
        if client.is_paused():
            client.resume()
    ...
    async def skip(self, ctx):
        ...
        if ctx.channel.permissions_for(
            ...
            await ctx.send(embed=discord.Embed(
                color=ctx.author.color,
                timestamp=ctx.message.created_at,
                description="Vote skipping is disabled"
            ))
    ...
    @music.command(aliases=["q", "playlist"])
    @commands.guild_only()
    @commands.check(audio_playing)
    async def queue(self, ctx):
        ...
        await ctx.send(embed=discord.Embed(
            color=ctx.author.color,
            timestamp=ctx.message.created_at,
            description=self._queue_text(state.playlist)
        ))
    ...
    @music.command(brief="Plays audio from <url>.", aliases=["p"])
    @commands.guild_only()
        ...
        if client and client.channel:
            ...
            if ctx.author.voice != None and ctx.author.voice.channel != None:
                ...
                await ctx.send(embed=discord.Embed(
                    color=ctx.author.color,
                    timestamp=ctx.message.created_at,
                    description="You need to be in a voice channel to do that"
                ))
